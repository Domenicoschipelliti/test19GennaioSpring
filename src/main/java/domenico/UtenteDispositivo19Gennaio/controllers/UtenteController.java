...
public class UtenteController {
    ...
    //GET
    @GetMapping
    public Page<Utente> utenti(@RequestParam(defaultValue = "0")int page,@RequestParam(defaultValue = "20")int size,@RequestParam(defaultValue = "id")String order){
       return utenteService.findAll(page,size,order);
    }
    //GET(ID)
    @GetMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    public Utente utente(@PathVariable UUID id){
